...
- `hardware/`: Chứa firmware ESP32 cho phần cứng thật (TTGO T-Beam), gồm code Xi node (đọc cảm biến BME280, MAX30102, GPS, mã hóa AES, giao tiếp LoRa) và Y Gateway (quét Beacon, gửi ACK, chuyển tiếp HTTP).
...
3. Đọc dữ liệu từ các cảm biến (nhiệt độ, độ ẩm, áp suất, nhịp tim, SpO2, CO, CO2, NH3, GPS).
...
- Khởi tạo I2C (GPIO21-SDA, GPIO22-SCL) cho OLED, BME280, MAX30102.
- Khởi tạo OLED SSD1306 để hiển thị trạng thái.
- Khởi tạo cảm biến BME280 (địa chỉ 0x76) đo nhiệt độ, độ ẩm, áp suất.
- Khởi tạo MAX30102 (địa chỉ 0x57) đo nhịp tim và SpO2.
...
**MAX30102 — Nhịp tim, SpO2 (dòng 114-150):**
```cpp
bool doc_max30102(SensorData& d) {
  // Lấy 25 mẫu IR trong 5 giây
  // Phát hiện nhịp tim bằng checkForBeat()
  // Tính BPM trung bình 4 nhịp gần nhất
  // SpO2 = 104.0 - 17.0 * (red/ir)
...
int   hr = random(2) ? 65 + random(20) : 0; // 0 hoặc 65-85 bpm
float spo2 = hr ? 95.0 + random(50)/10.0 : 0; // 95-100%
```
            ...
            conn.execute('''
                INSERT INTO telemetry
                    (device_id, temperature, humidity, pressure,
                     co2, co, nh3, heart_rate, spo2, altitude,
...
Các chỉ số hiển thị: Nhiệt độ, Độ ẩm, Áp suất, Nhịp tim, SpO2, CO2, CO/NH3.
...
| Dữ liệu cảm biến (nhiệt độ, độ ẩm, nhịp tim, v.v.) | Wokwi dùng dữ liệu ngẫu nhiên trong khoảng hợp lý (25-31°C, 58-62%, v.v.) — đủ để kiểm thử toàn bộ luồng mã hóa, truyền tin và hiển thị |
...
| Dữ liệu cảm biến đang dùng giá trị giả lập | Dữ liệu chưa phải giá trị đo từ môi trường thật. Việc giả lập với khoảng giá trị hợp lý vẫn đủ để kiểm thử luồng mã hóa, truyền tin, lưu trữ và hiển thị | Tích hợp cảm biến thật (BME280, MAX30102) khi có phần cứng |
...
1. **Node Xi** (ESP32 + cảm biến): Code firmware đọc cảm biến BME280, MAX30102, GPS; thực hiện giao thức Beacon → ACK → Data; mã hóa AES-128-CBC bằng hardware accelerator mbedtls (~17µs); gửi dữ liệu qua LoRa.
